...
class UM_User_Tags_API {
	...
	function init() {
		...
		require_once um_user_tags_path . 'includes/core/filters/um-user-tags-members.php';
	...
	function get_tags( $user_id, $metakey ) {
		...
		if ( is_array( $tags ) ) {
			foreach ( $tags as $tag ) {
				...
				$term = $this->get_localized_term_by( $tag );
				if ( $term ) {
					$i++;
		...
		if ( $remaining > 0 ) {
			$value .= '<span class="um-user-tag um-user-tag-more"><a href="javascript:void(0);">' . sprintf(__('%s more', 'um-user-tags'), $remaining) . '</a></span>';
		}
