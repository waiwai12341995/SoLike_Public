...
class Messaging_GDPR {
	...
	function render_download_chat_link( $message_to, $user_id ) {
		...
		<a href="<?php echo $this->get_download_url( $response['conversation_id'] ) ?>" data-conversation_id="<?php echo ! empty( $response['conversation_id'] ) ? $response['conversation_id'] : 'new'; ?>"
		...
		if ( $_GET['conversation_id'] == 'all' ) {
			...
			if ( ! empty( $conversations ) ) {
				foreach ( $conversations as $conversation ) {
					$messages = wp_cache_get( "um_conversation_messages:{$conversation->conversation_id}", 'um_messaging' );
					if ( false === $messages ) {
						...
						wp_cache_set( "um_conversation_messages:{$conversation->conversation_id}", $messages, 'um_messaging' );
					}
			...
			$messages = wp_cache_get( "um_conversation_messages:{$_GET['conversation_id']}", 'um_messaging' );
			if ( false === $messages ) {
				$messages = $wpdb->get_results( $wpdb->prepare(
					"SELECT * 
					FROM {$wpdb->prefix}um_messages
				...
				wp_cache_set( "um_conversation_messages:{$_GET['conversation_id']}", $messages, 'um_messaging' );
			}
